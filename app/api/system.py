...
@router.post("/menus")
async def list_menus(user_id: int = Depends(get_current_user_id), session: AsyncSession = Depends(get_session)):
    menus = await get_all_menus(session)
    return success_response(menus)


@router.post("/menu/add")
async def add_menu(request: MenuCreateRequest, user_id: int = Depends(get_current_user_id), session: AsyncSession = Depends(get_session)):
...
async def update_menu_route(request: MenuUpdateRequest, user_id: int = Depends(get_current_user_id), session: AsyncSession = Depends(get_session)):
    data = request.model_dump(exclude_unset=True, exclude_none=True)
...
async def delete_menu_route(id: int, user_id: int = Depends(get_current_user_id), session: AsyncSession = Depends(get_session)):
