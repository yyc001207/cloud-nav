...
轻量级导航页后端服务，基于 FastAPI 构建，提供用户认证、导航管理、文件上传、代理服务、系统管理、中转站和 OpenList STRM 生成等核心能力。
...
│   │   ├── verify.py        # Token 验证路由
│   │   └── __init__.py
│   ├── business/            # 业务逻辑层
│   │   ├── auth/            # 认证业务（schema, service）
│   │   ├── user/            # 用户管理业务
│   │   ├── nav/             # 导航管理业务
│   │   ├── system/          # 系统管理业务
│   │   ├── upload/          # 文件上传业务
...
本项目需要连接外部 MySQL 和 Redis 容器，通过 Docker 网络通信。确保 MySQL 和 Redis 的 compose 文件使用相同的网络名称：
...
> **注意**：网络名称需与 `docker-compose.yml` 中的 `external` 网络名称一致。

### 目录映射

| 容器路径       | 宿主机路径          | 说明             |
| -------------- | ------------------- | ---------------- |
| /app/uploads   | ./prod/uploads      | 上传文件目录     |
| /app/logs      | ./prod/logs         | 日志目录         |
| /app/output    | C:/Users/.../Videos/emby | STRM 输出目录 |
...
| 路径                     | 功能         | 认证 |
| ------------------------ | ------------ | ---- |
| POST /api/user/userInfo  | 获取用户信息 | 是   |
...
| 路径                         | 功能             | 认证 |
| ---------------------------- | ---------------- | ---- |
| POST /api/system/menus       | 获取菜单列表     | 是   |
...
| POST /api/proxy/holidays       | 获取节假日列表（支持分页、排序）   | 是   |
| POST /api/proxy/holiday/query  | 按年份查询节假日                   | 是   |
...
| 路径                              | 功能                               | 认证 |
| --------------------------------- | ---------------------------------- | ---- |
| POST /api/transfer/text/list      | 获取文本列表（支持分页、搜索、排序） | 是   |
...
| POST /api/transfer/file/list      | 获取文件列表（支持分页、搜索、排序） | 是   |
| POST /api/transfer/file/delete    | 删除文件中转                       | 是   |
| POST /api/transfer/file/download  | 下载文件（支持断点续传）           | 是   |
...
| POST /api/openlist/task-config/list     | 获取任务配置列表（支持分页、搜索、排序） | 是   |
...
| POST /api/openlist/tasks/running        | 获取运行中任务列表           | 是   |
| POST /api/openlist/task/latest-results  | 获取各任务最新执行结果       | 是   |
| POST /api/openlist/task/history         | 获取指定任务执行历史         | 是   |
...
所有需要认证的接口通过 **HttpOnly Cookie** 传递 JWT Token，Cookie 名称为 `token`：

### 登录流程
1. 用户登录成功后，服务器会通过 `Set-Cookie` 响应头设置 Token Cookie
2. Cookie 属性：
   - HttpOnly：防止 JavaScript 访问，增加安全性
   - SameSite=Lax：防止 CSRF 攻击
   - Path=/：适用于所有路径
   - Max-Age：根据 Token 过期时间设置
   - Domain（可选）：根据配置设置
   - Secure（可选）：生产环境建议启用

### Token 验证
所有需要认证的请求会自动携带 Cookie，服务器从 Cookie 中读取 Token 进行验证。

### Token 黑名单
登出时，Token 会被加入 Redis 黑名单，支持服务端强制失效。
...
| 分页响应格式 | `{code, msg, data: {list, total, page, size, total_pages}}` | **`{code, msg, data, total}`**         |
